...
export class LogWrapper implements ILogWrapper {
  ...
  logMessage (msg: RawMessage, selfInfo: SelfInfo) {
    ...
    const base = `${isSelfSent ? '发送 ->' : '接收 <-'} ${rawMessageToText(msg)}`;

    const grayTipElements = msg.elements.filter((element) => element.elementType === ElementType.GreyTip && element.grayTipElement);
    if (grayTipElements.length === 0) {
      this.log(base);
      return;
    }

    // Grey tip messages can be frequent; log only the raw payload string (jsonStr/xml) to avoid log spam and freezes.
    for (const element of grayTipElements) {
      const grayTip = element.grayTipElement as any;
      const subElementType = grayTip?.subElementType;
      ...
      if (typeof jsonStr === 'string' && jsonStr.length > 0) {
        this.log(base, `[灰条 jsonStr subElementType=${subElementType}${busiId ? ` busiId=${busiId}` : ''}]`, jsonStr.trimEnd());
        continue;
      }

      const xmlContent = grayTip?.xmlElement?.content;
      if (typeof xmlContent === 'string' && xmlContent.length > 0) {
        const templId = grayTip?.xmlElement?.templId ?? '';
        this.log(base, `[灰条 xml subElementType=${subElementType}${templId ? ` templId=${templId}` : ''}${busiId ? ` busiId=${busiId}` : ''}]`, xmlContent);
      }
    }
  }
}
