...
import { ElementType, KickedOffLineInfo, RawMessage, SelfInfo, SelfStatusInfo } from '@/napcat-core/types';
...
export class NapCatCore {
  ...
  private readonly uidToUinCache = new Map<string, { uin: string; expiresAt: number; }>();
  private readonly uidToUinInflight = new Map<string, Promise<string>>();
  private readonly uidToUinCacheMaxSize = 2000;
  private readonly uidToUinCacheHitTtlMs = 10 * 60 * 1000;
  private readonly uidToUinCacheMissTtlMs = 60 * 1000;
  ...
  async initNapCatCoreListeners () {
    ...
    msgListener.onRecvMsg = (msgs) => {
      msgs.forEach((msg) => {
        ...
        void this.logGreyTipSenderUin(msg);
      });
    };
  ...
  private hasGreyTipElement (msg: RawMessage): boolean {
    return msg.elements.some((element) => element.elementType === ElementType.GreyTip && element.grayTipElement);
  }

  private extractGreyTipSenderUid (msg: RawMessage): string | undefined {
    const senderUid = msg.senderUid?.trim();
    if (senderUid && senderUid !== '0') {
      return senderUid;
    }

    for (const element of msg.elements) {
      if (element.elementType !== ElementType.GreyTip || !element.grayTipElement) continue;
      const grayTip = element.grayTipElement as any;
      const jsonStr = grayTip?.jsonGrayTipElement?.jsonStr;
      if (typeof jsonStr !== 'string' || jsonStr.length === 0) continue;
  ...
  private setUidToUinCache (uid: string, uin: string, ttlMs: number) {
    if (this.uidToUinCache.size >= this.uidToUinCacheMaxSize) {
      const firstKey = this.uidToUinCache.keys().next().value as string | undefined;
      if (firstKey) {
        this.uidToUinCache.delete(firstKey);
      }
    }
    this.uidToUinCache.set(uid, { uin, expiresAt: Date.now() + ttlMs });
  }

  private resolveUinByUidCached (uid: string): Promise<string> {
    const now = Date.now();
    const cached = this.uidToUinCache.get(uid);
    if (cached && cached.expiresAt > now) {
      return Promise.resolve(cached.uin);
    }

    const inflight = this.uidToUinInflight.get(uid);
    if (inflight) {
      return inflight;
    }

    const promise = this.apis.UserApi.getUinByUidV2(uid)
      .then((uin) => {
        const resolved = uin ?? '0';
        this.uidToUinInflight.delete(uid);
        this.setUidToUinCache(uid, resolved, resolved === '0' ? this.uidToUinCacheMissTtlMs : this.uidToUinCacheHitTtlMs);
        return resolved;
      })
      .catch(() => {
        this.uidToUinInflight.delete(uid);
        this.setUidToUinCache(uid, '0', this.uidToUinCacheMissTtlMs);
        return '0';
      });

    this.uidToUinInflight.set(uid, promise);
    return promise;
  }

  private logGreyTipSenderUin (msg: RawMessage): void {
    if (!this.hasGreyTipElement(msg)) return;
    if (msg.senderUin && msg.senderUin !== '0') return;

    const senderUid = this.extractGreyTipSenderUid(msg);
    if (!senderUid) return;

    void this.resolveUinByUidCached(senderUid).then((uin) => {
      if (!uin || uin === '0') return;

      const firstGreyTip = msg.elements.find((element) => element.elementType === ElementType.GreyTip && element.grayTipElement);
      const grayTip = firstGreyTip?.grayTipElement as any;
      const busiId = grayTip?.jsonGrayTipElement?.busiId ?? grayTip?.xmlElement?.busiId ?? '';
      this.context.logger.log(`[灰条 sender qq=${uin}${busiId ? ` busiId=${busiId}` : ''}]`);
    });
  }
}
