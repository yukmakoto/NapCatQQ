  ...
  private initializeServer () {
    ...
    this.app.use((req, _res, next) => {
      if (this.config.debug) {
        this.core.context.logger.logDebug('[OneBot] [HTTP Server Adapter] Incoming Request', {
          method: req.method,
          url: req.originalUrl,
    ...
    this.app.use((req, res, next) => {
      ...
      req.on('end', () => {
        try {
          (req as Request & { rawBody?: string }).rawBody = rawData;
          if (this.config.debug) {
            this.core.context.logger.logDebug('[OneBot] [HTTP Server Adapter] Incoming Body', truncateForLog(rawData, 20000));
          }
