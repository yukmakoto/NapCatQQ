...
function redactRawHeaders (rawHeaders: readonly string[]) {
  const sensitive = new Set(['authorization', 'cookie', 'set-cookie', 'x-signature']);
  const result: string[] = [];
  for (let i = 0; i < rawHeaders.length; i += 2) {
    const name = rawHeaders[i] ?? '';
    const value = rawHeaders[i + 1] ?? '';
    result.push(name);
    result.push(sensitive.has(name.toLowerCase()) ? '[redacted]' : value);
  }
  return result;
}
...
export class OB11WebSocketServerAdapter extends IOB11NetworkAdapter<WebsocketServerConfig> {
  ...
  createServer (newServer: WebSocketServer) {
    newServer.on('connection', async (wsClient, wsReq) => {
      ...
      if (this.config.debug) {
        this.logger.logDebug('[OneBot] [WebSocket Server] Client Connected', {
          url: wsReq.url,
          remoteAddress: wsReq.socket.remoteAddress,
          rawHeaders: redactRawHeaders(wsReq.rawHeaders),
        });
      }
  ...
  private async handleMessage (wsClient: WebSocket, message: RawData) {
    ...
    try {
      ...
      if (this.config.debug) {
        this.logger.logDebug('[OneBot] [WebSocket Server] Incoming Message', truncateForLog(raw, 20000));
      }
      receiveData = json5.parse(raw);
